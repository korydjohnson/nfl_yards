...
toRemove = c(1:12, 14:16, 18, 20:29, 33)
...
tibble(theResponse) %>% 
  mutate(
    probEst = map2(pred, sd, getProbs.norm, rangeY=seq(-99,99)),
...
rangeY_norm = log(pmax(seq(-99,99) + 15, 1))
(pred = mean(theResponse2))
(sd = sd(theResponse2))
tibble(theResponse2) %>% 
  mutate(
    probEst = map2(pred, sd, getProbs.norm),
    crps = map2_dbl(probEst, theResponse2, crps, rangeY)
  ) %>% 
  summarise(crps = mean(crps))
