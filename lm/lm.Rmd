...
toRemove = c(1:12, 14:16, 18, 20:29, 33, 48)
theData = loadData("features_py.csv") %>% 
  select(-toRemove) %>% 
  mutate_at(vars(categorical), as.factor)
colnames(theData)
# theData = select(theData, -c(7, 9:10, 12:21,24:30))  # for simpler model
...
```{r}
predictCV = function(theResponse, theData, modelFnc, k=5, rangeY) {
...
qqnorm(theResponse2)
theResponse2 = log(pmax(theResponse + 5, 1))
...
plot(lmOut)
rangeY = log(pmax(rangeY_true + 16, 1))
...
distPreds = 
  ...
  mutate(
    ...
    probsTrans = map(pred, getProbs.trans, rangeY=rangeY_true),
