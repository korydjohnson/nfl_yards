...
class Preprocessor:
    ...
    def __init__(self, x, feat_num, feat_cat):
        self.scaler = StandardScaler()
        self.feat_num = feat_num
        self.feat_cat = feat_cat
        self.scaler.fit(x[self.feat_num])

    def transform_data(self, x, y, test):
        ...
        if test:
            ...
            y = torch.from_numpy(pd.get_dummies(pd.Categorical(y.iloc[:,0] + 99, range(199))).cumsum(axis=1).values.astype(float)).float()
            return x, y
...
class nn2(nn.Module):
    ...
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size, batch_size, n_unique):
