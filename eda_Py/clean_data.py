...
dropColumns = ["GameClock", "DisplayName", "JerseyNumber", "NflId", "Season",
               "NflIdRusher", "TimeHandoff", "TimeSnap", "PlayerBirthDate",
               "PlayerCollegeName", "Stadium", "Location", "WindSpeed", "WindDirection",
               "HomeScoreBeforePlay", "VisitorScoreBeforePlay", "Humidity", "Temperature"]

# fixing abbreviations
map_Teams = {"ARZ": "ARI", "BLT": "BAL", "CLV": "CLE", "HST": "HOU"}
...
map_StadiumType = {nameList[i][j]: newNames[i] for i in range(len(newNames))
                   for j in range(len(nameList[i]))}

# DefendersInTheBox
defenders_imputeVal = pd.read_csv('../input/train.csv', usecols=["DefendersInTheBox"]).\
    DefendersInTheBox.mean()
...
map_Turf = {nameList[i][j]: newNames[i] for i in range(len(newNames))
            for j in range(len(nameList[i]))}
...
map_GameWeather = {nameList[i][j]: newNames[i] for i in range(len(newNames))
                   for j in range(len(nameList[i]))}


def create_categoricals(df):  # Encode the team values
    columns = ["PossessionTeam", "FieldPosition", "HomeTeamAbbr", "VisitorTeamAbbr"]
    for col in columns:
        df[col].map(map_Teams)  # clean names
